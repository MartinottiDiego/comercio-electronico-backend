...
export class PushNotificationService {
  ...
  private constructor() {
    ...
    if (!publicKey || !privateKey) {
      ...
      webpush.setVapidDetails(
        'mailto:notifications@waazaar.com',
        publicKey,
        privateKey
      );
    }
  ...
  /**
   * Enviar notificación push a múltiples suscripciones
   */
   ...
   */
  async sendPushNotificationToRole(
    role: 'comprador' | 'tienda' | 'admin',
    data: PushNotificationData
  ): Promise<{ success: number; failed: number }> {
    try {
