  ...
  async getAutomationStats(ctx: Context) {
    try {
      ...
      const stats = {
        ...
        disabled: triggers.filter(t => !t.enabled).length,
        byType: {
          growth: triggers.filter(t => t.insightType === 'growth').length,
          revenue: triggers.filter(t => t.insightType === 'revenue').length,
          engagement: triggers.filter(t => t.insightType === 'engagement').length,
          conversion: triggers.filter(t => t.insightType === 'conversion').length,
          category_trends: triggers.filter(t => t.insightType === 'category_trends').length,
